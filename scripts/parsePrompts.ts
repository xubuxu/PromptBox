 ...
 * Run with: npx ts-node scripts/parsePrompts.ts
 */
 ...
 */
function generateDefaultPromptsFile(prompts: ParsedPrompt[], outputPath: string): void {
    const content = `/**
...
// Main execution
const promptsFilePath = join(__dirname, '..', 'PROMPTS.md');
const outputFilePath = join(__dirname, '..', 'src', 'main', 'defaultPrompts.ts');
